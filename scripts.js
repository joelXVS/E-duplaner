...
function toggleSideBar() {
  ...
  if (span) span.textContent = isSidebarExpanded ? "Mostrar NavBar" : "Ocultar NavBar";
...
function agregarDocente(event) {
  ...
  if (!nombre) {
    agregarNotificacion('warning', "Por favor ingrese el nombre del docente");
    return false;
  }
  ...
  for (const ag of asignaturasGrupos) {
    ...
    for (const grupoId of ag.grupos) {
      ...
      if (!verificacion.valido) {
        agregarNotificacion('warning', verificacion.mensaje);
        return false;
      }
    }
  }
  ...
  if (totalHorasAsignadas > maxHoras) {
    agregarNotificacion('warning', `El docente tendría asignadas ${totalHorasAsignadas} horas, pero su máximo es ${maxHoras}.`);
    return false;
  }
  ...
  agregarNotificacion('success', "Docente agregado correctamente");
...
function agregarAsignatura(event) {
  ...
  if (datos.asignaturas.some(a => a.identificador === identificador)) {
    agregarNotificacion('warning', "Ya existe una asignatura con ese identificador");
    return false;
  }
  ...
  if (gradosHoras.length === 0) {
    agregarNotificacion('warning', "Debe seleccionar al menos un grado");
    return false;
  }
  ...
  agregarNotificacion('success', "Asignatura agregada correctamente");
...
function agregarGrupo(event) {
  ...
  if (datos.grupos.some(g => g.grado === grado && g.nombre === nombre)) {
    agregarNotificacion('warning', "Ya existe un grupo con ese nombre en el grado seleccionado");
    return false;
  }
  ...
  // Mostrar notificación
  agregarNotificacion('success', "Grupo agregado correctamente");
...
function agregarGrado(event) {
  ...
  if (datos.grados.some(g => g.nombre === nombre)) {
    agregarNotificacion('warning', "Ya existe un grado con ese nombre");
    return false;
  }
  ...
  // Mostrar notificación
  agregarNotificacion('success', "Grado agregado correctamente");
...
function agregarArea(event) {
  ...
  if (datos.areas.some(a => a.nombre === nombre)) {
    agregarNotificacion('warning', "Ya existe un área con ese nombre");
    return false;
  }
  ...
  // Mostrar notificación
  agregarNotificacion('success', "Área agregada correctamente");
...
function agregarHorarioBase(event) {
  ...
  if (dias.length === 0) {
    agregarNotificacion('warning', "Debe seleccionar al menos un día de la semana");
    return false;
  }
  ...
  if (grupos.length === 0) {
    agregarNotificacion('warning', "Debe seleccionar al menos un grupo");
    return false;
  }
  ...
  // Mostrar notificación
  agregarNotificacion('success', "Horario base agregado correctamente");
...
function guardarEdicionDocenteCompleta() {
  ...
  // Mostrar notificación
  agregarNotificacion('success', "Docente actualizado correctamente");
}
...
function guardarEdicionAsignatura() {
  ...
  if (datos.asignaturas.some(a => a.id !== asignaturaId && a.identificador === identificador)) {
    agregarNotificacion('warning', "Ya existe otra asignatura con ese identificador");
    return;
  }
  ...
  // Mostrar notificación
  agregarNotificacion('success', "Asignatura actualizada correctamente");
}
...
function guardarEdicionGrupo() {
  ...
  if (datos.grupos.some(g => g.id !== grupoId && g.grado === grado && g.nombre === nombre)) {
    agregarNotificacion('warning', "Ya existe otro grupo con ese nombre en el grado seleccionado");
    return;
  }
  ...
  // Mostrar notificación
  agregarNotificacion('success', "Grupo actualizado correctamente");
}
...
function guardarEdicionGrado() {
  ...
  if (datos.grados.some(g => g.id !== gradoId && g.nombre === nombre)) {
    agregarNotificacion('warning', "Ya existe otro grado con ese nombre");
    return;
  }
  ...
  // Mostrar notificación
  agregarNotificacion('success', "Grado actualizado correctamente");
}
...
function guardarEdicionArea() {
  ...
  if (datos.areas.some(a => a.id !== areaId && a.nombre === nombre)) {
    agregarNotificacion('warning', "Ya existe otra área con ese nombre");
    return;
  }
  ...
  // Mostrar notificación
  agregarNotificacion('success', "Área actualizada correctamente");
}
...
function guardarEdicionHorarioBase() {
  ...
  // Mostrar notificación
  agregarNotificacion('success', "Horario base actualizado correctamente");
}
...
function cargarTablaAsignaciones() {
  ...
  document.getElementById("buscar-asignacion").addEventListener("input", filtrarAsignaciones)
}
...
function cargarHorariosBase() {
  ...
  datos.horariosBase.forEach(horarioBase => {
    ...
    // Verificar si hay grupos con asignaciones incompletas
    const gruposIncompletos = verificarAsignacionesCompletas();
    const tieneGruposIncompletos = gruposIncompletos.some(g => 
      horarioBase.grupos.includes(g.id)
    );
    ...
    card.innerHTML = `
      <div class="horario-base-header">
        ...
        <div class="horario-base-actions">
          <button class="btn-icon" onclick="editarHorarioBase('${horarioBase.id}')"><i class="fas fa-edit"></i></button>
          <button class="btn-icon" onclick="confirmarEliminarHorarioBase('${horarioBase.id}')"><i class="fas fa-trash"></i></button>
        </div>
      </div>
      ...
      ${tieneGruposIncompletos ? `
        <div class="validation-alert" style="margin: 10px 0;">
          <i class="fas fa-exclamation-triangle"></i>
          <div class="alert-content">
            <p>Hay grupos con asignaciones incompletas. Complete las asignaciones antes de generar horarios.</p>
          </div>
        </div>
      ` : ''}
      ...
      <div class="horario-base-footer">
        ...
        <div class="horario-base-buttons">
          <button class="btn" onclick="iniciarGeneracionManual('${horarioBase.id}')">
            <i class="fas fa-pencil-alt"></i> Manual
          </button>
          <button class="btn" onclick="iniciarGeneracionAutomatica('${horarioBase.id}')">
...
function finalizarGuardadoManual(horarioBase, horarios) {
  ...
  // Mostrar mensaje de éxito
  agregarNotificacion('success', "Horarios guardados correctamente");
...
function eliminarGeneracion(historialId) {
  ...
  // Mostrar notificación
  agregarNotificacion('success', 'Generación eliminada correctamente');
}
...
function toggleModoOscuro() {
  ...
  agregarNotificacion('info', `Modo ${isDarkMode ? 'oscuro' : 'claro'} activado`);
}
...
function borrarTodosLosDatos() {
  ...
  // Mostrar notificación
  agregarNotificacion('success', 'Todos los datos han sido borrados. Se ha creado un backup automático por si necesita recuperarlos.');
}
...
function exportarDatosJSON() {
  ...
  if (datos.docentes.length === 0 && datos.asignaturas.length === 0 && datos.grupos.length === 0) {
    agregarNotificacion('warning', 'No hay datos para exportar. Agregue al menos un docente, asignatura o grupo.');
    return;
  }
  ...
  // Notificar al usuario
  agregarNotificacion('success', 'Datos exportados correctamente');
}
...
function crearBackup(mensaje = 'Backup creado correctamente') {
  ...
  if (datos.docentes.length === 0 && datos.asignaturas.length === 0 && datos.grupos.length === 0) {
    agregarNotificacion('warning', 'No hay datos para crear un backup. Agregue al menos un docente, asignatura o grupo.');
    return null;
  }
  ...
  // Notificar al usuario
  agregarNotificacion('success', mensaje);
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Inicializar tabs para la sección Escuela
  inicializarTabs();
  ...
  // Añadir eventos a los inputs de búsqueda
  const buscarInputs = document.querySelectorAll('input[type="search"]');
  buscarInputs.forEach(input => {
    input.addEventListener('input', function() {
      const id = this.id;
      if (id === "buscar-docente") {
        filtrarDocentes();
      } else if (id === "buscar-asignatura") {
        filtrarAsignaturas();
      } else if (id === "buscar-grupo") {
        filtrarGrupos();
      } else if (id === "buscar-asignacion") {
        filtrarAsignaciones();
      }
    });
  });
  
  // Añadir eventos a los selects de filtro
  const filtroSelects = document.querySelectorAll('select[id^="filtro-"]');
  filtroSelects.forEach(select => {
    select.addEventListener('change', function() {
      const id = this.id;
      if (id === "filtro-area-asignatura") {
        filtrarAsignaturas();
      } else if (id === "filtro-grado") {
        filtrarGrupos();
      } else if (id === "filtro-grupo-asignacion" || id === "filtro-docente-asignacion") {
        filtrarAsignaciones();
      }
    });
  });
  ...
  // Agregar botones de funcionalidades adicionales al panel de control
  ...
  const elementosCriticos = [
    "docente-form", "asignatura-form", "grupo-form", "grado-form", "area-form", "horario-base-form",
    "form-nuevo-docente", "form-nueva-asignatura", "form-nuevo-grupo", "form-nuevo-grado", "form-nueva-area", "form-nuevo-horario-base"
  ];
  
  elementosCriticos.forEach(id => {
    if (!elementoExiste(id)) {
      console.warn(`Elemento crítico ${id} no encontrado. Algunas funciones podrían no trabajar correctamente.`);
    }
  });
});
...
// Agrega estas funciones para el sistema de notificaciones
function mostrarNotificacion(notificacion) {
  ...
  switch (notificacion.tipo) {
    ...
    case 'error': icono = 'fas fa-times-circle'; break;
    default: icono = 'fas fa-info-circle'; // info
  }
    ...
    <div class="notificacion-contenido">
      ...
      ${notificacion.accion ? `
        <div class="notificacion-accion">
          <button onclick="${notificacion.accion.funcion}('${notificacion.id}')">
...
// Función para agregar los estilos de notificaciones
function agregarEstilosNotificaciones() {
  const styleElement = document.createElement('style');
  styleElement.textContent = estilosNotificaciones;
  document.head.appendChild(styleElement);
}